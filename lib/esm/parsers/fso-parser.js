...
class FSOParser extends _parser.Parser {
  async doParse() {
  ...
  async classify() {
    ...
    for (let i = 0; i < typeIDs.length; i++) {
      ...
      graph.push(...(await (0, _classAssignment.buildClassInstances)(this.ifcAPI, this.modelID, typeID, fsoClass, true)));
    }
  ...
  async portPort() {
    ...
    return await (0, _pathSearch.buildRelOneToOne)(this.ifcAPI, this.modelID, IFCRELCONNECTSPORTS, subjectRef, targetRef, rdfRelationship, false, true);
  ...
  async portComponent() {
    ...
    const r1 = await (0, _pathSearch.buildRelOneToOne)(this.ifcAPI, this.modelID, IFCRELCONNECTSPORTTOELEMENT, subjectRef, targetRef, rdfRelationship);
    subjectRef = "RelatingPort";
    targetRef = "RelatedElement";
    rdfRelationship = "fso:connectedComponent";
    const r2 = await (0, _pathSearch.buildRelOneToOne)(this.ifcAPI, this.modelID, IFCRELCONNECTSPORTTOELEMENT, subjectRef, targetRef, rdfRelationship);
    return r1.concat(r2);
  }
  /**
   * PROPERTIES
  ...
  async portFlowDirection(expressIDArray) {
  ...
  async portPlacements(expressIDArray) {
    const graph = [];

    for (let i = 0; i < expressIDArray.length; i++) {
      const expressID = expressIDArray[i];
      const props = await this.ifcAPI.properties.getItemProperties(this.modelID, expressID, true);
      const coordinates = await (0, _objectPlacement.getGlobalPosition)(props.ObjectPlacement);
      const point = `POINT Z(${coordinates[0]} ${coordinates[1]} ${coordinates[2]})`;
      const portURI = (0, _uriBuilder.defaultURIBuilder)(props.GlobalId.value);
  ...
  async getPortIDs() {
    ...
    const subTypes = (0, _classAssignment.getElementSubtypes)(IFCPORT); // Get all items in model that belong to any of these types
