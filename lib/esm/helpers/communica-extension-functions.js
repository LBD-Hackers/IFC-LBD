"use strict";

Object.defineProperty(exports, "__esModule", {
  value: true
});
exports.geoSPARQLFunctions = exports.extensionFunctions = void 0;

var _rdfDataFactory = require("rdf-data-factory");

var _wktParser = require("./wkt-parser");

const DF = new _rdfDataFactory.DataFactory();
const extensionFunctions = {
  'http://example.org/functions#to-upper-case'(args) {
    const arg = args[0];

    if (arg.termType === 'Literal' && arg.datatype.value === 'http://www.w3.org/2001/XMLSchema#string') {
      return DF.literal(arg.value.toUpperCase(), arg.datatype);
    }

    return arg;
  },

  'http://example.org/functions#get-id'(args) {
    const arg = args[0];

    if (arg.termType === 'NamedNode') {
      return DF.literal(getID(arg));
    }

    return arg;
  },

  'http://example.org/functions#get-namespace'(args) {
    const arg = args[0];

    if (arg.termType === 'NamedNode') {
      return DF.literal(getNamespace(arg));
    }

    return arg;
  },

  'http://example.org/functions#uri-concat'(args) {
    const uri1 = args[0];
    const uri2 = args[1];

    if (uri1.termType === 'NamedNode' && uri2.termType === 'NamedNode') {
      const ns = getNamespace(uri1);
      const id1 = getID(uri1);
      const id2 = getID(uri2);
      return DF.namedNode(ns + id1 + id2);
    }

    return DF.literal("ERROR");
  }

};
exports.extensionFunctions = extensionFunctions;
const geoSPARQLFunctions = {
  'http://www.opengis.net/def/function/geosparql/distance'(args) {
    if (args[0].termType === 'Literal' && args[1].termType === 'Literal' && args.length == 2) {
      const p1 = (0, _wktParser.parseWKT)(args[0].value);
      console.log(p1);
      return DF.literal("3", DF.namedNode('http://www.w3.org/2001/XMLSchema#decimal'));
    }

    return DF.literal("ERROR");
  }

};
exports.geoSPARQLFunctions = geoSPARQLFunctions;

function getID(uri) {
  return uri.value.indexOf("#") != -1 ? uri.value.split("#")[1] : uri.value.split("/").pop();
}

function getNamespace(uri) {
  if (uri.value.indexOf("#") != -1) {
    return uri.value.split("#")[0] + "#";
  } else {
    const arr = uri.value.split("/");
    arr.pop();
    return arr.join("/") + "/";
  }
}
//# sourceMappingURL=communica-extension-functions.js.map